...
namespace QFSW.QC
{
    ...
    public class CommandData
    {
        ...
        public CommandData(MethodInfo methodData, int defaultParameterCount = 0) : this(methodData, methodData.Name,
        ...
        public CommandData(MethodInfo methodData, string commandName, int defaultParameterCount = 0)
        {
            ...
            if (string.IsNullOrWhiteSpace(commandName)) CommandName = methodData.Name;

            var declaringType = methodData.DeclaringType;

            var prefix = BuildPrefix(declaringType);
            ...
            {
                var j = MethodParamData.Length - defaultParameterCount + i;
        ...
        public CommandData(MethodInfo methodData, CommandAttribute commandAttribute, int defaultParameterCount = 0) :
            this(methodData, commandAttribute.Alias, defaultParameterCount)
        {
            CommandDescription = commandAttribute.Description;
            MonoTarget = commandAttribute.MonoTarget;
        }

        public CommandData(MethodInfo methodData, CommandAttribute commandAttribute,
            CommandDescriptionAttribute descriptionAttribute, int defaultParameterCount = 0)
        ...
        public Type[] MakeGenericArguments(params Type[] genericTypeArguments)
        {
            ...
            var substitutionTable = new Dictionary<string, Type>();
            ...
            for (var i = 0; i < types.Length; i++)
                ...
                {
                    var substitution = ConstructGenericType(ParamTypes[i], substitutionTable);
        ...
        private Type ConstructGenericType(Type genericType, Dictionary<string, Type> substitutionTable)
        {
            ...
            {
                var baseType = genericType.GetGenericTypeDefinition();
        ...
        {
            var data = new object[paramData.Length + _defaultParameters.Length];
            ...
            var invokingMethod = GetInvokingMethod(genericTypeArguments);

            if (IsStatic) return invokingMethod.Invoke(null, data);

            var targets = InvocationTargetFactory.FindTargets(invokingMethod.DeclaringType, MonoTarget);
        ...
        {
            if (!IsGeneric) return MethodData;
            ...
            T WrapConstruction<T>(Func<T> f)
            {
                ...
                catch (ArgumentException)
                {
                    throw new ArgumentException(
                        $"Supplied generic parameters did not satisfy the generic constraints imposed by '{CommandName}'");
                }
            }
            ...
            {
                var typeCount = declaringType.GetGenericArguments().Length;

                var genericTypes = genericTypeArguments
                    .Take(typeCount)
                    .ToArray();
        ...
        private string BuildPrefix(Type declaringType)
        {
            ...
            {
                foreach (var prefixAttribute in prefixAttributes.Reverse())
            ...
            {
                var typePrefixes = declaringType.GetCustomAttributes<CommandPrefixAttribute>();
                AddPrefixes(typePrefixes, declaringType.Name);

                declaringType = declaringType.DeclaringType;
            }

            var assemblyPrefixes = assembly.GetCustomAttributes<CommandPrefixAttribute>();
        ...
        {
            if (genericParamTypes.Length == 0) return string.Empty;

            var names = genericParamTypes.Select(x => x.Name);
        ...
        private string BuildParameterSignature(ParameterInfo[] methodParams, int defaultParameterCount)
        {
            ...
            for (var i = 0; i < methodParams.Length - defaultParameterCount; i++)
        ...
        private Type[] BuildGenericParamTypes(MethodInfo method, Type declaringType)
        {
            ...
            if (declaringType.IsGenericTypeDefinition) types.AddRange(declaringType.GetGenericArguments());

            if (method.IsGenericMethodDefinition) types.AddRange(method.GetGenericArguments());
