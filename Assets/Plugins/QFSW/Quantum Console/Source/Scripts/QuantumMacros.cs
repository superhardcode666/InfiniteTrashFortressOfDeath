...
namespace QFSW.QC
{
    public static class QuantumMacros
    {
        ...
        /// <param name="maximumExpansions">
        ///     The maximum number of macro expansions that can be performed before an exception is
        ///     thrown.
        /// </param>
        public static string ExpandMacros(string text, int maximumExpansions = 1000)
        {
            ...
            for (var i = 0; i < text.Length; i++)
                if (text[i] == '#')
                    foreach (var macro in _macroTable)
                    {
                        var keyLength = macro.Key.Length;
                        if (i + keyLength < text.Length)
                            if (text.Substring(i + 1, keyLength) == macro.Key)
                            {
                                if (expansionCount >= maximumExpansions)
                                    throw new ArgumentException(
                                        $"Maximum macro expansions of {maximumExpansions} was exhausted: infinitely recursive macro is likely.");

                                var start = text.Substring(0, i);
                                var end = text.Substring(i + 1 + keyLength);
        ...
        [Command("#define")]
        [CommandDescription(
            "Adds a macro to the macro table which can then be used in the Quantum Console. If the macro 'name' is added, " +
        ...
        {
            using (var dumpFile = new StreamWriter(filePath))
            {
                foreach (var macro in _macroTable) dumpFile.WriteLine($"{macro.Key} {macro.Value}");
        ...
        public static string LoadMacrosFromFile(string filePath)
        {
            ...
            using (var macroFile = new StreamReader(filePath))
            {
                var messages = new List<string>();
                while (!macroFile.EndOfStream)
                {
                    var line = macroFile.ReadLine();
                    var parts = line.Split(" ".ToCharArray(), 2);
                    if (parts.Length != 2) messages.Add($"'{line}' is not a valid macro definition");
        ...
        private class MacroPreprocessor : IQcPreprocessor
        {
            ...
            {
                if (!text.StartsWith("#define", StringComparison.CurrentCulture)) text = ExpandMacros(text);
