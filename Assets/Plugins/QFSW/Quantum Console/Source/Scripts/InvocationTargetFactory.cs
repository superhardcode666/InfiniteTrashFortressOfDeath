...
namespace QFSW.QC
{
    ...
    {
        private static readonly Dictionary<(MonoTargetType, Type), object> TargetCache =
            new Dictionary<(MonoTargetType, Type), object>();

        public static IEnumerable<T> FindTargets<T>(MonoTargetType method) where T : MonoBehaviour
        {
            foreach (var target in FindTargets(typeof(T), method)) yield return target as T;
        }
        ...
        public static IEnumerable<object> FindTargets(Type classType, MonoTargetType method)
        {
            switch (method)
            {
                ...
                {
                    var target = Object.FindObjectOfType(classType);
        ...
        private static IEnumerable<object> WrapSingleCached(Type classType, MonoTargetType method,
            Func<Type, object> targetFinder)
        {
            if (!TargetCache.TryGetValue((method, classType), out var target) || target as Object == null)
            {
        ...
        public static object InvokeOnTargets(MethodInfo invokingMethod, IEnumerable<object> targets, object[] data)
        {
            var returnCount = 0;
            var invokeCount = 0;
            var resultsParts = new Dictionary<object, object>();

            foreach (var target in targets)
            {
                invokeCount++;
                var result = invokingMethod.Invoke(target, data);
            ...
            if (returnCount > 1) return resultsParts;

            if (returnCount == 1) return resultsParts.Values.First();

            if (invokeCount == 0)
            {
                var typeName = invokingMethod.DeclaringType.GetDisplayName();
                throw new Exception(
        ...
        {
            var obj = new GameObject($"{classType}Singleton");
