...
namespace QFSW.QC
{
    ...
    public class QuantumParser
    {
        ...
        private readonly ConcurrentDictionary<Type, IQcParser> _parserLookup =
            new ConcurrentDictionary<Type, IQcParser>();
        ...
        /// </summary>
        public QuantumParser() : this(new InjectionLoader<IQcParser>().GetInjectedInstances(),
            new InjectionLoader<IQcGrammarConstruct>().GetInjectedInstances())
        {
        }

        public IQcParser GetParser(Type type)
        {
            if (_parserLookup.ContainsKey(type)) return _parserLookup[type];
            ...
            if (!_unparseableLookup.Contains(type))
            {
                foreach (var parser in _parsers)
                    try
                    {
                        if (parser.CanParse(type)) return _parserLookup[type] = parser;
                    }
                    catch (Exception e)
                    {
        ...
        {
            foreach (var grammar in _grammarConstructs)
                try
                {
                    if (grammar.Match(value, type)) return grammar;
                }
                catch (Exception e)
                {
        ...
        public object Parse(string value, Type type)
        {
            ...
            if (type.IsClass && value == "null") return null;

            var grammar = GetMatchingGrammar(value, type);
            if (grammar != null)
                try
                {
            ...
            if (parser == null) throw new ArgumentException($"Cannot parse object of type '{type.GetDisplayName()}'");
        ...
        private static readonly string[] _defaultNamespaces =
            {"System", "System.Collections", "System.Collections.Generic", "UnityEngine", "UnityEngine.UI", "QFSW.QC"};
        ...
        {
            if (!_namespaceTable.Contains(namespaceName)) _namespaceTable.Add(namespaceName);
        }
        ...
        public static Type ParseType(string typeName)
        {
            ...
            if (_reverseTypeDisplayNames.ContainsKey(typeName)) return _reverseTypeDisplayNames[typeName];

            if (_tupleTypeRegex.IsMatch(typeName)) return ParseTupleType(typeName);

            if (_arrayTypeRegex.IsMatch(typeName)) return ParseArrayType(typeName);

            if (_genericTypeRegex.IsMatch(typeName)) return ParseGenericType(typeName);

            if (typeName.Contains('`'))
            {
                var genericName = typeName.Split('`')[0];
                if (_reverseTypeDisplayNames.ContainsKey(genericName)) return _reverseTypeDisplayNames[genericName];
            }
        ...
        {
            var arrayPos = typeName.LastIndexOf('[');
            var arrayRank = typeName.CountFromIndex(',', arrayPos) + 1;
            var elementType = ParseType(typeName.Substring(0, arrayPos));
        ...
        private static Type ParseGenericType(string typeName)
        {
            ...
            var genericArgNames = $"<{parts[1]}".ReduceScope('<', '>').SplitScoped(',');
            var incompleteGenericName = $"{parts[0]}`{Math.Max(1, genericArgNames.Length)}";

            var incompleteGenericType = ParseType(incompleteGenericName);
            if (genericArgNames.All(string.IsNullOrWhiteSpace)) return incompleteGenericType;

            var genericArgs = genericArgNames.Select(ParseType).ToArray();
        ...
        private static Type CreateTupleType(Type[] types)
        {
            ...
            {
                var innerTypes = types.Skip(maxFlatTupleSize - 1).ToArray();
                types = types
        ...
        private static Type GetTypeFromAssemblies(string typeName, IEnumerable<string> namespaces,
            IEnumerable<Assembly> assemblies, bool throwOnError, bool ignoreCase)
        {
            foreach (var namespaceName in namespaces)
            {
                var type = GetTypeFromAssemblies($"{namespaceName}.{typeName}", assemblies, false, ignoreCase);
                if (type != null) return type;
            }

            if (throwOnError)
                throw new TypeLoadException(
                    $"No type of name '{typeName}' could be found in the specified assemblies and namespaces.");

            return null;
        }

        private static Type GetTypeFromAssemblies(string typeName, IEnumerable<Assembly> assemblies, bool throwOnError,
            bool ignoreCase)
        {
            foreach (var assembly in assemblies)
            {
                var type = Type.GetType($"{typeName}, {assembly.FullName}", false, ignoreCase);
                if (type != null) return type;
            }

            if (throwOnError)
                throw new TypeLoadException(
                    $"No type of name '{typeName}' could be found in the specified assemblies.");
