        ...
        public FieldAutoMethod(FieldInfo fieldInfo, AccessType accessType) : base(fieldInfo)
        {
            ...
            if (_accessType == AccessType.Read)
            {
                if (_fieldInfo.IsStatic)
                    _internalDelegate = (Func<FieldInfo, object>) _StaticReader;
                else
                    _internalDelegate = (Func<object, object>) _fieldInfo.GetValue;
                _parameters = Array.Empty<ParameterInfo>();
            }
            else
            {
                if (_fieldInfo.IsStatic)
                    _internalDelegate = (Action<FieldInfo, object>) _StaticWriter;
                else
                    _internalDelegate = (Action<object, object>) _fieldInfo.SetValue;
