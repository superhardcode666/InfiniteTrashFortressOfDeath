    ...
    public static class EditorHelpers
    {
        private static readonly SupportItem[] _supportItems =
        {
            ...
            new SupportItem
            {
                Name = "Survey",
                Tooltip =
                    "A short survey to help me get feedback on Quantum Console and prioritize what needs the most focus.",
        ...
        public static void DrawBanner(Texture2D banner, float sizeMultiplier = 1f)
        {
            if (banner)
            {
                ...
                var bannerRect = GUILayoutUtility.GetRect(0.0f, 0.0f);
        ...
        {
            var layout = new LayoutController(EditorGUILayout.GetControlRect());
            layout.SpliceRow(_supportItemRects.Length, ref _supportItemRects);

            for (var i = 0; i < _supportItems.Length; i++)
            {
                var item = _supportItems[i];
