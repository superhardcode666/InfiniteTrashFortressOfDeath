...
namespace QFSW.QC.Editor
{
    ...
    public class QuantumThemeInspector : QCInspectorBase
    {
        ...
        protected override void OnEnable()
        {
            ...
            _typeFormattersListDisplay =
                new ReorderableList(serializedObject, _typeFormattersProperty, true, true, true, true);
            ...
            _collectionFormattersListDisplay = new ReorderableList(serializedObject, _collectionFormattersProperty,
                true, true, true, true);
        ...
        private void CreateStyles()
        {
            if (!_initialisedStyles)
            {
                ...
                _centeredMiniLabel.normal.textColor = EditorGUIUtility.isProSkin
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField(new GUIContent("UI", "Theme customisations for the Quantum Console UI."),
                EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(_fontProperty,
                new GUIContent("Font", "The font to be used throughout the Quantum Console."));

            EditorGUILayout.PropertyField(_panelMaterialProperty,
                new GUIContent("Panel Material", "The material to use in the UI panels. Leave null for default."));
            EditorGUILayout.PropertyField(_panelColorProperty,
                new GUIContent("Panel Color", "The color to use in the UI panels."));
            EditorGUILayout.Space();

            EditorGUILayout.LabelField(
                new GUIContent("Colors", "Color customisation for various aspects of the Quantum Console."),
                EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(_errorColorProperty,
                new GUIContent("Error Color", "The color to use when formatting errors in the console log."));
            EditorGUILayout.PropertyField(_warningColorProperty,
                new GUIContent("Warning Color", "The color to use when formatting warnings in the console log."));
            EditorGUILayout.PropertyField(_successColorProperty,
                new GUIContent("Success Color", "The color to use when formatting successful void commands."));
            EditorGUILayout.PropertyField(_selectedSuggestionColorProperty,
                new GUIContent("Selected Suggestion Color",
                    "The color to use for the selected suggestion from the suggestion popup display."));
            EditorGUILayout.PropertyField(_suggestionColorProperty,
                new GUIContent("Suggestion Signature Color",
                    "The color to use when displaying the paramater signature for suggested commands."));
            EditorGUILayout.PropertyField(_commandLogColorProperty,
                new GUIContent("Command Log Color",
                    "The color to use when displaying logged commands in the console log."));
            EditorGUILayout.Space();

            EditorGUILayout.LabelField(
                new GUIContent("Formatting", "Control various formatting within Quantum Console."),
                EditorStyles.boldLabel);
            ...
            EditorGUILayout.LabelField(
                new GUIContent("Return Value Formatting", "Formatting options for the return serialization"),
                EditorStyles.boldLabel);
            EditorGUILayout.PropertyField(_defaultValueColorProperty,
                new GUIContent("Default Color", "The default color for return values"));
        ...
        private void DrawTypeFormatterInspector(Rect drawRect, int index, bool isActive, bool isFocused)
        {
            ...
            var currentTypeFormatter = _typeFormattersListDisplay.serializedProperty.GetArrayElementAtIndex(index);
            var colorProperty = currentTypeFormatter.FindPropertyRelative("Color");

            var typeName = _themeInstance.TypeFormatters[index].Type.GetDisplayName();
            var labelRect = new Rect(drawRect.x, drawRect.y, nameWidth, EditorGUIUtility.singleLineHeight);
            var colorRect = new Rect(drawRect.x + nameWidth, drawRect.y, drawRect.width - nameWidth,
        ...
        {
            EditorGUI.LabelField(new Rect(drawRect.x, drawRect.y, 150, drawRect.height),
                new GUIContent("Type Formatters",
                    "The different colors that should be used for formatting different type returns."));
            EditorGUI.LabelField(new Rect((drawRect.x + drawRect.width) / 2 - 40, drawRect.y, 80, drawRect.height),
                _typeFormattersProperty.arraySize +
                (_typeFormattersProperty.arraySize == 1 ? " Formatter" : " Formatters"), _centeredMiniLabel);
        }

        private void AppendNewTypeFormatter(ReorderableList listTarget)
        {
            Action<string> SubmitCallback = data =>
            {
                var type = QuantumParser.ParseType(data);
                if (type == null)
                    throw new ArgumentException(
                        $"No type of name '{data}' could be found. Are you missing a namespace?");
            ...
            PopupWindow.Show(new Rect(5, 5, 0, 0),
                new DataEntryPopup("Type Name", "Create Type Formatter", SubmitCallback));
        }
        ...
        private void DrawCollectionFormatterInspector(Rect drawRect, int index, bool isActive, bool isFocused)
        {
            ...
            var padding = 5f;
            var endPadding = 10f;
            var nameWidth = drawRect.width - (6 * itemWidth + 3 * dataWidth + 5 * padding + endPadding);
            drawRect.y += 2;

            var currentCollectionFormatter =
                _collectionFormattersListDisplay.serializedProperty.GetArrayElementAtIndex(index);
            var seperatorProperty = currentCollectionFormatter.FindPropertyRelative("SeperatorString");
            var leftScoperProperty = currentCollectionFormatter.FindPropertyRelative("LeftScoper");
            var rightScoperProperty = currentCollectionFormatter.FindPropertyRelative("RightScoper");

            var typeName = _themeInstance.CollectionFormatters[index].Type.GetDisplayName();
            var rect = new Rect(drawRect.x, drawRect.y, nameWidth, EditorGUIUtility.singleLineHeight);
            EditorGUI.LabelField(rect, typeName);
            rect.x += nameWidth + padding;

            Action<SerializedProperty, float> DrawTextField = (prop, width) =>
            {
                rect.width = width;
            ...
            Action<string, float> DrawLabelField = (text, width) =>
            {
                rect.width = width;
            ...
            var highlightCol = EditorGUIUtility.isProSkin ? "#1fe035" : "#005209";
            var itemCol = EditorGUIUtility.isProSkin ? "#ff8280" : "#6A0301";
            var example = $"<b><color={highlightCol}>{leftScoperProperty.stringValue}</color></b>" +
        ...
        {
            EditorGUI.LabelField(new Rect(drawRect.x, drawRect.y, 150, drawRect.height),
                new GUIContent("Collection Formatters",
                    "The different strings that should be used for seperating and enclosing collections when serialized."));
            EditorGUI.LabelField(new Rect((drawRect.x + drawRect.width) / 2 - 40, drawRect.y, 80, drawRect.height),
                _collectionFormattersProperty.arraySize +
                (_collectionFormattersProperty.arraySize == 1 ? " Formatter" : " Formatters"), _centeredMiniLabel);
        }
        ...
        private void AppendNewCollectionFormatter(ReorderableList listTarget)
        {
            ...
            PopupWindow.Show(new Rect(5, 5, 0, 0),
                new DataEntryPopup("Collection Type Name", "Create Collection Formatter", SubmitCallback));
        }
    }
}