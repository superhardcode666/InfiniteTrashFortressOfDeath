...
namespace QFSW.QC.Utilities
{
    ...
    {
        private static readonly ConcurrentDictionary<int, string> _colorLookupTable =
        ...
        public static string ColorText(this string text, Color color)
        {
            ...
            var hexColor = Color32ToStringNonAlloc(color);
            return $"<color=#{hexColor}>{text}</color>";
        }

        public static unsafe string Color32ToStringNonAlloc(Color32 color)
        {
            var colorKey = (color.r << 24) | (color.g << 16) | (color.b << 8) | color.a;
            if (_colorLookupTable.ContainsKey(colorKey)) return _colorLookupTable[colorKey];

            var buffer = stackalloc char[8];
            Color32ToHexNonAlloc(color, buffer);

            var bufferLength = color.a < 0xFF ? 8 : 6;
