...
namespace QFSW.QC
{
    public static class TextProcessing
    {
            ...
            where T : IReadOnlyList<char>
        {
            ...
            if (maxReduction == 0) return trimmedInput;

            for (var i = 0; i < leftScopers.Count; i++)
            {
                var leftScoper = leftScopers[i];
                var rightScoper = rightScopers[i];

                if (leftScoper == rightScoper)
                {
                    var descopedInput = trimmedInput;
                    var descoped = false;
                    ...
                    for (var j = 0; j < descopedInput.Length; j++)
                ...
                else
                {
                    ...
                    {
                        var scope = 1;
                        for (var j = 1; j < trimmedInput.Length - 1; j++)
                        {
                            if (trimmedInput[j] == leftScoper)
                                scope++;
                            else if (trimmedInput[j] == rightScoper) scope--;

                            if (scope == 0) return trimmedInput;
                        }

                        return trimmedInput.Substring(1, trimmedInput.Length - 2)
                            .ReduceScope(leftScopers, rightScopers, maxReduction - 1);
                    }
                }
            }
        ...
        public static string[] SplitScoped(this string input, char splitChar, int maxCount,
            bool autoReduceScope = false)
        {
        ...
        public static string[] SplitScoped(this string input, char splitChar, char leftScoper, char rightScoper,
            int maxCount, bool autoReduceScope = false)
        {
            return input.SplitScoped(splitChar, leftScoper.AsArraySingle(), rightScoper.AsArraySingle(), maxCount,
                autoReduceScope);
        }
            ...
            where T : IReadOnlyList<char>
        {
            ...
            var rawSplitIndices = GetScopedSplitPoints(input, splitChar, leftScopers, rightScopers);
            int[] splitIndices;
            if (maxCount > 0)
            ...
            if (splitIndices.Length == 0) return new[] {input};

            var splitString = new string[splitIndices.Length + 1];
        ...
        public static IEnumerable<int> GetScopedSplitPoints<T>(string input, char splitChar, T leftScopers,
            T rightScopers)
            where T : IReadOnlyList<char>
        {
            if (leftScopers.Count != rightScopers.Count)
            ...
            var scopes = new int[leftScopers.Count];
            for (var i = 0; i < input.Length; i++)
            {
                if (i == 0 || input[i - 1] != '\\')
                    for (var j = 0; j < leftScopers.Count; j++)
                    {
                        var leftScoper = leftScopers[j];
                        var rightScoper = rightScopers[j];

                        if (input[i] == leftScoper && leftScoper == rightScoper)
                            scopes[j] = 1 - scopes[j];
                        else if (input[i] == leftScoper)
                            scopes[j]++;
                        else if (input[i] == rightScoper) scopes[j]--;
                    }

                if (input[i] == splitChar && scopes.All(x => x == 0)) yield return i;
            }
        }
        ...
        {
            foreach (var escapeChar in escapeChars) input = input.Replace($"\\{escapeChar}", escapeChar.ToString());
