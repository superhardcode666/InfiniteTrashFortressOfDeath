...
namespace QFSW.QC.Actions
{
    ...
    public class Typewriter : Composite
    {
        ...
        private static IEnumerator<ICommandAction> Generate(string message, Config config)
        {
            ...
            {
                case Config.ChunkType.Character:
                    chunks = message.Select(c => c.ToString()).ToArray();
                    break;
                case Config.ChunkType.Word:
                    chunks = WhiteRegex.Split(message);
                    break;
                case Config.ChunkType.Line:
                    chunks = LineRegex.Split(message);
                    break;
