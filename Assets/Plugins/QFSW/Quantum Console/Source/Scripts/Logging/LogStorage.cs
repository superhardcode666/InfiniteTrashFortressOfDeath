...
namespace QFSW.QC
{
    public class LogStorage : ILogStorage
    {
        ...
        public void AddLog(ILog log)
        {
            ...
            var logLength = _logTraceBuilder.Length + log.Text.Length;
            if (log.NewLine && _logTraceBuilder.Length > 0) logLength += Environment.NewLine.Length;

            if (MaxStoredLogs > 0)
                while (_consoleLogs.Count > MaxStoredLogs)
                {
                    var junkLength = _consoleLogs[0].Text.Length;
                    if (_consoleLogs.Count > 1 && _consoleLogs[1].NewLine) junkLength += Environment.NewLine.Length;
            ...
            while (capacity < logLength) capacity *= 2;

            _logTraceBuilder.EnsureCapacity(capacity);

            if (log.NewLine && _logTraceBuilder.Length > 0) _logTraceBuilder.Append(Environment.NewLine);
        ...
        public void RemoveLog()
        {
            ...
            {
                var log = _consoleLogs[_consoleLogs.Count - 1];
                _consoleLogs.RemoveAt(_consoleLogs.Count - 1);

                var removeLength = log.Text.Length;
                if (log.NewLine && _consoleLogs.Count > 0) removeLength += Environment.NewLine.Length;
