...
namespace QFSW.QC
{
    ...
    public static partial class QuantumConsoleProcessor
    {
        ...
        private static readonly ConcurrentDictionary<string, CommandData> _commandTable =
            new ConcurrentDictionary<string, CommandData>();

        private static int _loadedCommandCount;
        ...
        /// <param name="fuzzy">
        ///     If fuzzy search is disabled, then your current search must match the beginning of the command to be
        ///     suggested (foo*). If fuzzy search is enabled, it can be anywhere within the command name to be suggested (*foo*).
        /// </param>
        ...
        public static IEnumerable<CommandData> GetCommandSuggestions(string incompleteCommandName, bool fuzzy = false,
            bool caseSensitive = true, bool includeOverloads = true)
        {
            if (string.IsNullOrWhiteSpace(incompleteCommandName)) return Enumerable.Empty<CommandData>();

            var foundNames = includeOverloads ? null : new HashSet<string>();
            var foundCommands = new List<CommandData>();
            ...
            bool MatchFound(string incompleteCommand, string commandSuggestion)
            {
                ...
                {
                    if (caseSensitive)
                        return commandSuggestion.Contains(incompleteCommand);
                    return commandSuggestion.ContainsCaseInsensitive(incompleteCommand);
                }
            ...
            {
                if (caseSensitive)
                    sortedSuggestions = sortedSuggestions.OrderBy(x =>
                        x.CommandName.IndexOf(incompleteCommandName, StringComparison.CurrentCulture));
                else
                    sortedSuggestions = sortedSuggestions.OrderBy(x =>
                        x.CommandName.IndexOf(incompleteCommandName, StringComparison.CurrentCultureIgnoreCase));
            }
            ...
            lock (_commandTable)
            {
                if (!TableGenerated || forceReload)
                {
                    TableIsGenerating = true;
                    {
                        ...
                        {
                            if (AssemblyRequiresScan(assembly)) LoadCommandsFromAssembly(assembly);
                        });
#else
        ...
        private static IEnumerable<(MethodInfo method, MemberInfo member)> ExtractCommandMethods(Type type)
        {
            ...
            foreach (var method in methods) yield return (method, method);

            foreach (var property in properties)
            {
                if (property.CanWrite) yield return (property.SetMethod, property);
                if (property.CanRead) yield return (property.GetMethod, property);
            }
            ...
            foreach (var field in fields)
                if (field.HasAttribute<CommandAttribute>())
                {
                    if (field.IsDelegate())
                    {
                        ...
                        {
                            var executer = new FieldDelegateMethod(field);
                            yield return (executer, field);
                        }
                        else if (loggingLevel >= LoggingLevel.Warnings)
                        {
                            Debug.LogWarning(
                                $"Quantum Processor Warning: Could not add '{field.Name}' from {field.DeclaringType} to the table as it is an invalid delegate type.");
                        }
                    }
                    else
                    {
                        var reader = new FieldAutoMethod(field, FieldAutoMethod.AccessType.Read);
                        yield return (reader, field);

                        if (!(field.IsLiteral || field.IsInitOnly))
                        {
                            var writer = new FieldAutoMethod(field, FieldAutoMethod.AccessType.Write);
        ...
        {
            for (var i = 0; i < command.ParamCount; i++)
            {
                var param = command.MethodParamData[i];
            ...
            {
                unsupportedReason =
                    $"Non static non MonoBehaviour commands are incompatible with MonoTargetType.{command.MonoTarget}.";
                return false;
            }
        ...
        {
            if (assembly.HasAttribute<QcIgnoreAttribute>(false)) return false;
        ...
        private static void LoadCommandsFromType(Type type)
        {
            ...
            if (type.HasAttribute<CompilerGeneratedAttribute>()) return;

            foreach (var (method, member) in ExtractCommandMethods(type))
        ...
        {
            var commandAttributes = member.GetCustomAttributes<CommandAttribute>();
            var descriptionAttribute = member.GetCustomAttribute<CommandDescriptionAttribute>();
            ...
            foreach (var commandAttribute in commandAttributes)
                if (!commandAttribute.Valid)
                {
                    if (loggingLevel >= LoggingLevel.Warnings)
                        Debug.LogWarning(
                            $"Quantum Processor Warning: Could not add '{commandAttribute.Alias}' to the table as it is invalid.");
                }
                else
                {
                    var platformAttribute = member.GetCustomAttribute<CommandPlatformAttribute>();
                    var commandPlatforms = platformAttribute?.SupportedPlatforms ?? commandAttribute.SupportedPlatforms;
                    if (commandPlatforms.HasFlag(Application.platform.ToPlatform()))
                    {
                        var newCommands = CreateCommandOverloads(method, commandAttribute, descriptionAttribute);
                        foreach (var command in newCommands) TryAddCommand(command);
                    }
                }
        }

        private static IEnumerable<CommandData> CreateCommandOverloads(MethodInfo method,
            CommandAttribute commandAttribute, CommandDescriptionAttribute descriptionAttribute)
        {
            var defaultParameters = method.GetParameters().Count(x => x.HasDefaultValue);
            for (var i = 0; i < defaultParameters + 1; i++)
            {
                var command = new CommandData(method, commandAttribute, descriptionAttribute, i);
        ...
        {
            if (!GetCommandSupported(command, out var reason))
            {
                if (loggingLevel >= LoggingLevel.Warnings)
                    Debug.LogWarning(
                        $"Quantum Processor Warning: Could not add '{command.CommandSignature}' from {command.MethodData.DeclaringType.GetDisplayName()} " +
                        $"to the table as it is not supported. {reason}");

                return false;
            }

            var key = GenerateCommandKey(command);
            var alreadyExists = !_commandTable.TryAdd(key, command);

            if (alreadyExists)
            {
                if (loggingLevel >= LoggingLevel.Warnings)
                {
                    var fullMethodName = $"{command.MethodData.DeclaringType.FullName}.{command.MethodData.Name}";
                    Debug.LogWarning(
                        $"Quantum Processor Warning: Could not add {fullMethodName} to the table as another method with the same alias and parameter count, {key}, already exists.");
                }
        ...
        public static object InvokeCommand(string commandString)
        {
            ...
            var commandParts = commandString.SplitScoped(' ');
            commandParts = commandParts.Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();

            var commandName = commandParts[0];
            var commandParams = commandParts.SubArray(1, commandParts.Length - 1);
            var paramCount = commandParams.Length;

            var commandNameParts = commandName.Split(new[] {'<'}, 2);
            var genericSignature = commandNameParts.Length > 1 ? $"<{commandNameParts[1]}" : "";
            commandName = commandNameParts[0];

            var keyName = $"{commandName}({paramCount})";
            if (!_commandTable.ContainsKey(keyName))
            {
                var overloadExists = _commandTable.Keys.Any(key =>
                    key.Contains($"{commandName}(") && _commandTable[key].CommandName == commandName);
                if (overloadExists)
                    throw new ArgumentException(
                        $"No overload of '{commandName}' with {paramCount} parameters could be found.");
                throw new ArgumentException($"Command '{commandName}' could not be found.");
            }

            var command = _commandTable[keyName];

            var genericTypes = Array.Empty<Type>();
            if (command.IsGeneric)
            {
                var expectedArgCount = command.GenericParamTypes.Length;
                var genericArgNames = genericSignature.ReduceScope('<', '>').SplitScoped(',');
                ...
                {
                    throw new ArgumentException(
                        $"Generic command '{commandName}' requires {expectedArgCount} generic parameter{(expectedArgCount == 1 ? "" : "s")} but was supplied with {genericArgNames.Length}.");
                }
            }
            else if (genericSignature != string.Empty)
            {
                throw new ArgumentException(
                    $"Command '{commandName}' is not a generic command and cannot be invoked as such.");
            }
            ...
            var parsedCommandParams = ParseParamData(command.MakeGenericArguments(genericTypes), commandParams);
        ...
        {
            var parsedData = new object[paramData.Length];
            for (var i = 0; i < parsedData.Length; i++) parsedData[i] = _parser.Parse(paramData[i], paramTypes[i]);
