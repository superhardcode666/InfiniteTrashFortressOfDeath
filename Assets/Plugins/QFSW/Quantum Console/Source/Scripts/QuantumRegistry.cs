...
namespace QFSW.QC
{
    public static class QuantumRegistry
    {
        ...
        public static void ClearRegistryContents(Type type)
        {
            ...
            lock (_objectRegistry)
            {
                if (_objectRegistry.ContainsKey(type)) _objectRegistry.Clear();
            }
        }

        [Command("display-registry", "Displays the contents of the specified registry")]
        private static string DisplayRegistry(
            [CommandParameterDescription("Full namespace qualified name of the type of the registry.")]
            Type type)
        {
            var registrySize = GetRegistrySize(type);
            if (registrySize < 1)
                return $"registry '{type.GetDisplayName()}' was empty";
