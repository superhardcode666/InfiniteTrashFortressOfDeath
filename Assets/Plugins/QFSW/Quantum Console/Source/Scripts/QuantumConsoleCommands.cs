...
namespace QFSW.QC
{
    public static partial class QuantumConsoleProcessor
    {
        private const string helpStr =
            "Welcome to Quantum Console! In order to see specific help about any specific command, " +
        ...
        {
            return
                "To use the man command, simply put the desired command name in front of it. For example, 'man my-command' will generate the manual for 'my-command'";
        }

        [CommandDescription(
            "Generates a user manual for any given command, including built in ones. To use the man command, simply put the desired command name infront of it. For example, 'man my-command' will generate the manual for 'my-command'")]
        [Command("help")]
        [Command("manual")]
        [Command("man")]
        private static string GenerateCommandManual(string commandName)
        {
            var matchingCommands = _commandTable.Keys.Where(key => key.Split('(')[0] == commandName).OrderBy(key => key)
                .ToArray();
            if (matchingCommands.Length == 0)
                throw new ArgumentException($"No command with the name {commandName} was found.");

            var foundParams = new Dictionary<string, ParameterInfo>();
            var foundGenericArguments = new Dictionary<string, Type>();
            var foundParamDescriptions = new Dictionary<string, CommandParameterDescriptionAttribute>();
            var declaringTypes = new List<Type>(1);

            var manual = $"Generated user manual for {commandName}\nAvailable command signatures:";

            for (var i = 0; i < matchingCommands.Length; i++)
            {
                var currentCommand = _commandTable[matchingCommands[i]];
                ...
                for (var j = 0; j < currentCommand.ParamCount; j++)
                {
                    var param = currentCommand.MethodParamData[j];
                    if (!foundParams.ContainsKey(param.Name)) foundParams.Add(param.Name, param);
                    if (!foundParamDescriptions.ContainsKey(param.Name))
                    {
                        var descriptionAttribute = param.GetCustomAttribute<CommandParameterDescriptionAttribute>();
                ...
                {
                    var genericArgs = currentCommand.GenericParamTypes;
            ...
            {
                manual += "\nParameter info:";
                var commandParams = foundParams.Values.ToArray();
                for (var i = 0; i < commandParams.Length; i++)
                {
                    var currentParam = commandParams[i];
                    manual += $"\n   - {currentParam.Name}: {currentParam.ParameterType.GetDisplayName()}";
                }
            }

            var genericConstraintInformation = "";
            if (foundGenericArguments.Count > 0)
            {
                var genericArgs = foundGenericArguments.Values.ToArray();
                for (var i = 0; i < genericArgs.Length; i++)
                {
                    ...
                    var formattedConstraints = new List<string>();
            ...
            {
                manual += $" {declaringTypes[0].GetDisplayName(true)}";
            }
            else
            {
                manual += ":";
                foreach (var type in declaringTypes) manual += $"\n   - {type.GetDisplayName(true)}";
            }
        ...
        {
            var output = "List of all commands loaded by the Quantum Processor. Use 'man' on any command to see more:";
            foreach (var command in GetUniqueCommands()) output += $"\n   - {command.CommandName}";
