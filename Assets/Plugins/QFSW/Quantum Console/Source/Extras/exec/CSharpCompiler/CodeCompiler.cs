...
namespace CSharpCompiler
{
    public class CodeCompiler : ICodeCompiler
    {
        ...
        CompilerResults CompileFromCompilerSettings(CompilerSettings settings, bool generateInMemory)
        {
            ...
            var driver =
 new CustomDynamicDriver(new CompilerContext(settings, new CustomReportPrinter(compilerResults)));
        ...
        CompilerSettings ParamsToSettings(CompilerParameters parameters)
        {
            ...
            {
                parameters.OutputAssembly = settings.OutputFile =
 "DynamicAssembly_" + assemblyCounter + settings.TargetExt;
                assemblyCounter++;
            }
            settings.OutputFile =
 parameters.OutputAssembly; // if it is not being outputted, we use this to set name of the dynamic assembly
