...
public class CameraHolder : MonoBehaviour
{
    ...
    private readonly List<SVA> svList = new List<SVA>();
    ...
    private void OnGUI()
    {
        if (GUI.Button(new Rect(5 * windowDpi, 5 * windowDpi, 110 * windowDpi, 35 * windowDpi),
            "Previous effect")) Counter(-1);
        ...
        StartColor = HueColor;
        HueColor = GUI.HorizontalSlider(new Rect(5 * windowDpi, 45 * windowDpi, 340 * windowDpi, 35 * windowDpi),
            HueColor, 0, 1);
        GUI.DrawTexture(new Rect(5 * windowDpi, 65 * windowDpi, 340 * windowDpi, 15 * windowDpi), HueTexture,
        ...
        if (HueColor != StartColor)
        {
            ...
            {
                var main = ps.main;
                var colorHSV = Color.HSVToRGB(HueColor + H * 0, svList[i].S, svList[i].V);
    ...
    private void Counter(int count)
    {
        ...
        foreach (var ps in particleSystems)
        {
            var baseColor = ps.main.startColor.color;
            var baseSVA = new SVA();
    ...
    {
        if (currDistance < 2) currDistance = 2;
